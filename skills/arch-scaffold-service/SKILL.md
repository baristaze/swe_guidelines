...
| `src/<root>/services/<svc>/gateway/ratelimit.py` | `rate_limited(route)` reading the route's limit and window from one frozen options object built from settings at boot and held on the container, over `CacheInterface.increment` under the system scope, keyed on a digest of the bearer (the dependency runs before authentication) or the client address, failing open |
| `src/<root>/services/<svc>/gateway/idempotency.py` | the `Idempotency-Key` dependency over the OM's idempotency manager: `begin` before the call (a replay returns the stored response; a duplicate in flight is a conflict) and `finish` after it, keyed per tenant and user on a durable unique index; the cache is at most a read-through in front of it |
