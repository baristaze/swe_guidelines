...
| `src/<root>/services/<svc>/gateway/idempotency.py` | the `Idempotency-Key` dependency over the OM's idempotency manager: `begin` before the call (a replay returns the stored response; a duplicate in flight is a conflict) and `finish` after it, keyed per tenant and user on a durable unique index; the cache is at most a read-through in front of it. Every creating route (every `POST` that answers 201) in every hosted namespace declares it, not only the first namespace's, so a retried create returns the stored response |
