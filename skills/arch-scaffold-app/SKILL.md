...
| `src/api/client.ts`   | one transport client: the API origin taken from the app's config, bearer, app header, error envelope parsed into a typed error carrying the request id, 401 clears authentication |
...
| `src/config.ts`                                          | loads `/config.json` once before the first render (API origin, error-tracking DSN, environment name) and exposes it typed; no `import.meta.env` value that differs between environments |
...
| `.github/workflows/deploy.yml` (browser app) | the smaller environment's job builds the app once under the build id (the commit), keeps the bundle as a workflow artifact, syncs it to its bucket, writes that environment's `config.json` from the Terraform outputs, and invalidates the distribution; the production job, behind the approval gate, syncs the same build id's bundle to production's bucket and writes production's `config.json`, never building |
