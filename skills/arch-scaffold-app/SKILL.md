...
| File                  | Holds                                                                                   |
|-----------------------|-----------------------------------------------------------------------------------------|
| `openapi.json`        | at `apps/<app-name>/openapi.json`: the committed contract, a copy of the document of the service the app calls (`services/<service-name>/openapi.json`), regenerated by `make openapi` |
...
| `src/queries/keys.ts`, `src/queries/<domain>.ts`         | the query-key factory (entity name first in every key, so a write and a hint find every cached collection of an entity; a successful mutation writes its answer into them rather than re-reading them) and one TanStack Query hooks module per domain |
...
| `src/realtime/RealtimeProvider.tsx`, `envelopes.ts`, `router.ts`, `timeouts.ts` (portal only) | the one socket, opened on `/v1/realtime/socket?ticket=` with a ticket from `POST /v1/realtime/tickets` and subscribed to its tenant's stream by the service on open, so the client sends no `subscribe` for it; the discriminated union on `type` (mirroring the service's `envelopes.py`: `hello`, `pong`, `subscribed`, `unsubscribed`, `error`, and the `event` hint carrying `seq`, `kind`, `target_id`, and `actor_id` and no field of the entity); the router, which reads the one entity the hint's `kind` and `target_id` name through the authorized read and places it in the cached collections (a read that finds nothing removes it; a collection is read again only at a loaded page's edge or past a burst of hints; a late answer never overwrites a newer version); the last contiguous `seq` as the cursor, a `head_seq` on `hello` or on a `pong` above it being a gap like a missing `event` (a gap is a replay from it, never a skip), and the replay from `GET /v1/events?after_seq=` on reconnect or on a gap; the ping interval read from the shared timeouts file's `ping_interval_seconds` |
...
| `src/<root>/apps/<app>/listen.py` (when the API has a socket) | the feed command over the realtime channel, opened with `websockets` on `/v1/realtime/socket?ticket=` after `POST /v1/realtime/tickets` through the client, which is REST only, and which the service subscribes to the tenant's stream on open: it reads the `hello` frame's `head_seq`, replays from `GET /v1/events?after_seq=` up to it when it holds a cursor below it, keeps the last contiguous `seq` from each `event` hint, treats a `pong`'s `head_seq` above that cursor as a gap, and replays on a gap or a reconnect, never skipping; it prints each hint's `seq`, `kind`, `target_id`, and actor, the only fields a frame carries |
...
| `Makefile`                              | the `openapi` target copies the called service's `services/<service-name>/openapi.json` to `apps/<portal>/openapi.json` and runs `generate`, and regenerates `clients/python/` (already so on a tree `arch-scaffold-new` built); for a browser app, `check` also runs the workspace lint, typecheck, and test scripts, so CI's `make check` covers the app |
| `deployment/realtime-timeouts.json` (portal, when absent) | the ping interval and load balancer idle timeout, `ping_interval_seconds` and `idle_timeout_seconds`; the service scaffold writes it with `--realtime`, and the portal asserts it from the client side, in `timeouts.ts` and its test, the client half of the shared-file rule |
