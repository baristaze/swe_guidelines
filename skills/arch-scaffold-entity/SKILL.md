...
| `om/src/<root>/om/storage/roles.py`                     | `"<entities>": DatabaseRole.<ROLE>` in the table-to-role map, and `"<entities>": TenancyScope.<SCOPE>` in the tenancy scope map beside it, naming the column the policy rests on when the scope is `identity` or `both` |
