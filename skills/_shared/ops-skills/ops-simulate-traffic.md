...
repository, whose operator identity (`ACME_OPERATOR_EMAIL`,
`ACME_OPERATOR_PASSWORD` against `ACME_API_URL`) creates the tenants
...
4. Read one signal back to prove the run was seen: the request
   ...
   through `uv run acme-ops signals check --env <env> --request-id
   <one id from the run>`.
5. Write the report.
