...
| `om/tests/unit/test_work_manager.py`  | enqueue publishes, a retried enqueue leaves a claimed row untouched, the copy stamps the actor and the status over what the caller sent and keeps the timestamps as constructed, `enqueue_relayed` takes the actor and the request id from the row and meets the same insert as a retried `enqueue` under one `idempotency_key`, claim returns the enqueuer's context, which names the item's request as its cause, and the token, every write after the enqueue signs `updated_by` with `EMPTY_UUID`, complete and defer |
