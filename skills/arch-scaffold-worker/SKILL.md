...
allowed-tools: Read, Grep, Glob, Write, Edit, Bash(make check), Bash(uv run:*), Bash(uv sync:*), Bash(git status:*), Bash(git diff:*)
---
...
| `om/tests/unit/test_work_manager.py`  | enqueue publishes, a retried enqueue leaves a claimed row untouched, the copy stamps the actor and the status over what the caller sent and keeps the timestamps as constructed, claim returns the enqueuer's context and the token, complete and defer |
...
| `src/<root>/workers/<worker>/main.py`      | settings, the same `boot(settings)` as a service (logging, error reporting, trust store, tracing), `/metrics` and `/healthz` served alone on the metrics port, a small port of the worker's own since it has no API, `/healthz` reading the worker's own liveness key back and answering non-2xx when it is missing, container, stop handlers, the `serve` subcommand |
