...
| `storage/__init__.py`                 | `WorkStorageInterface`: `write_item`, `claim_next(lane, kinds, worker_id, lease)` as one method, `read_stale(before)`, `read_item`; the tenant-less methods (`claim_next`, `read_stale`, `read_item`) each carry a docstring saying why they take no tenant and return the tenant with each row |
...
| `om/tests/contracts/work_storage.py`, `om/tests/unit/test_work_storage.py`, `om/tests/integration/test_work_storage_postgres.py` | claim exclusivity, lease, requeue, a completion after the lease passed refused; the raced claim: two claimers at once against one item, exactly one wins, the same case over memory and over Postgres |
