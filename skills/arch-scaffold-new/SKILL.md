...
| File                                              | Holds                                                                                     |
|---------------------------------------------------|-------------------------------------------------------------------------------------------|
| `pyproject.toml`                                  | `[tool.uv] package = false`, `[tool.uv.workspace] members` (with a comment citing the root-package ADR by number), `[tool.uv.sources]`, pytest markers `integration`, `e2e`, `slow`, `telemetry` (needs the `devx` profile), and `[tool.arch-check]` with `package = "<root-package>"` and no other key |
...
| `Makefile`                                        | `setup` (`uv sync`, and `pnpm install` when a `package.json` exists), `infra-up`, `infra-down`, `infra-reset` (the dependencies recreated with their volumes removed and nothing else started, the step a gate and `arch-upgrade-deps` take where a developer takes `reset`), `migrate` (every role, `--all`), `migrate-check` (ORM metadata against the migrated schema, per role), `migrate-roundtrip` (downgrade the latest revision of every role, then upgrade it), `lint` (`ruff check`), `format-check` (`ruff format --check`), `typecheck` (`pyright`), `arch-check` (the guideline's static checker, `ARCH_CHECK ?= uvx --from "git+https://github.com/baristaze/swe_guidelines@v<version>\#subdirectory=checkers" arch-check` at the release found before writing, the same tag `specs/architecture.md` pins; `?=` so a developer offline points it at a local checkout), `check` (`lint`, `format-check`, `typecheck`, `arch-check`, `test-unit`), `test-unit`, `test-integration`, `openapi`, `devx-up` (the stack plus the `devx` profile; `infra-down` stops both), `seed` (the API process's `bootstrap --seed`), `up` (the dependencies and the `devx` profile in containers, then `migrate`, `seed`, the application on the host through `scripts/dev.sh`, and `urls`; the second compose file is for the case that asks for it and never the default), `down` (stops every container and the host processes, keeps the volumes), `reset` (`down` with the volumes removed, then `up`), `urls` (prints every local URL from `.env`), `test-telemetry` (the round-trip test under the `telemetry` marker, against the `devx` profile), `traffic` (`make traffic PROFILE=light DURATION=30`: the generator through the `<root>-ops` binary, the thirty-second light run being CI's wiring check), self-documented |
...
| `deployment/terraform/modules/`, `deployment/terraform/environments/{staging,prod}/` | one module per resource the settings name (database, cache, queue, buckets, secrets, service with rollout limits so a worker never exceeds its desired count, a log group with retention, and a non-essential OpenTelemetry collector beside each task that adds only service and environment as dimensions; the load balancer answers `/metrics` with a 404, served at `api.<base_domain>` with its certificate and DNS record), wired in both environments, each passing its `base_domain` (production the product's domain, staging a `staging.` subdomain of it), the API's allowed origins, and every prefix the settings read (buckets, queues, secrets) as variables, the process environment naming each of them, and the database URL wired as its own secret (never the password alone); environment names match the settings' cloud-environment set; the operations plane in the same graph: in `shared/`, the investigate role per environment, `<root>-investigate-<env>` (read-only over every signal and every resource description, the state prefix readable so `terraform plan -lock=false` runs, and fences denying every secret value, every data bucket's objects, the database connect, and the other environment's tags), the operators user `<root>-operators` whose one policy is `sts:AssumeRole` on those roles (no access key in Terraform), the budget (`monthly_budget_usd`, alerts at 50, 80, and 100 percent actual and 100 forecast to `owner_email`) and the anomaly monitor, and the DNS zone under `create_dns_zone`; per environment, the alarm topic `<root>-<env>-alarms` with its email subscription and the six alarms (load balancer 5xx ratio, unhealthy targets, database CPU, database free storage, running tasks below desired for the API and the worker, load balancer p95), the dashboard `<root>-<env>` with the same panels as the local metrics view, target-tracking autoscaling on every service behind the root switch `autoscaling_enabled` (off by default, every lever below it on, so one flip scales the environment), the root switch `destroyable` (off by default: `force_destroy` on the buckets, `skip_final_snapshot` and deletion protection off on the database), retention on every log group, and the environment tag through the provider's `default_tags` |
| `scripts/dev.sh`                                  | starts every application process on the host                                              |
| `scripts/cloud_create.sh`, `scripts/cloud_nuke.sh` | the hands of the create and nuke skills, both with `--dry-run` printing every command they would run: create takes `<env>` under the administrator profile `<root>-admin` and makes the state backend, applies `shared/`, mints the operators user's access key once and writes the profiles `<root>-operators` and `<root>-<env>-investigate` into `~/.aws/config`, sets the GitHub variables and environments, dispatches the first deploy through the pipeline, and runs the smoke test; nuke takes `<env>`, refuses `production` unless `--confirm production` is typed and the root's `database_deletion_protection` reads `false` on `main`, applies the `destroyable` switch, empties the data buckets, destroys the environment root, and prints what remains (the zone, the state prefix, `shared/`) |
| `.claude/skills/<name>/SKILL.md`, one per operational skill: `ops-investigate`, `ops-watch`, `ops-root-cause`, `ops-infra-as-code`, `ops-cloud-deployment-create`, `ops-cloud-deployment-nuke`, `ops-simulate-traffic`, `stress-test-create-or-update`, `stress-test-run` | copied from `${CLAUDE_SKILL_DIR}/../_shared/ops-skills/<name>.md` with every `acme` replaced by `<root>` (and `ACME` by its upper case, `Acme` by its capitalized form), nothing else edited: each names the profile it holds (`<root>-<env>-investigate` for every read, `<root>-admin` for create and nuke only), the owner-only env file `~/.config/<root>/ops/<env>.env`, what it reads, what it never does, and its report, and every one takes `--env local\|staging\|production`, `local` reading the `devx` twins |
| `ops/`                                            | the workspace member `<root>-ops`, package `<root>.ops`, binary `<root>-ops`: `traffic --env <env> --profile light\|regular\|heavy\|stress [--duration S] [--report path]`, the one generator, riding `clients/python/` and the operator plane, driving the edge and never a manager, with realistic sessions (sign in, list, add a handful, edit, complete, reopen, move, list, delete one, read events, one socket that sees its own change, sign out) and four profiles differing by tenants, members, concurrency, and think time, reporting requests by route and status, p50, p95, p99, and the error ratio; `stress --scenario ops/stress/<name>.yaml`, the same generator at the scenario's profile with its ramp, soak, and target; `signals check --env <env> --request-id <id>` over `SignalsInterface` (`log_lines`, `metric_delta`, `trace`, `error_event`) with `SignalsLocalImpl` (the Prometheus HTTP API, the Jaeger HTTP API, GlitchTip's REST API with the token the seed creates, the captured stdout) and `SignalsCloudImpl` (CloudWatch Logs Insights, `GetMetricData` on the product's namespace, X-Ray, the error tracker's REST API); `size --env <env>` (tenants, users, the main entity written in the last day, through `GET /v1/admin/size`); `ops/tests/test_telemetry_roundtrip.py` under the `telemetry` marker, which starts the API as a real process with the OTLP endpoint and the DSN set, drives one session, and reads every signal back by request id, so the same test runs as the deployed smoke test with a different base |
...
| `om/README.md`                                    | the nouns of the object model and how they relate, written for a reader with no code: what a tenant is, who a user and a member are, what the product's entities are and which belong to which, in the product's language; no developer instruction and no operator instruction in it |
...
| `src/<root>/om/storage/roles.py`       | `DatabaseRole`, the table-to-role map, `TenancyScope` (`system`, `org`, `identity`, `both`), and the table-to-scope map beside it, naming the column the policy rests on for an `identity` or `both` table, as The Storage Layer (The Second Fence) states |
...
7. When Docker is available, `make devx-up`, then
   ...
   thirty-second light run. Both are a wiring check of the edge, the
