...
| `src/<root>/om/outbox/`                | the transactional outbox of Database Roles: the `core`-role row (destination, payload, done), the named atomic storage method a namespace storage calls to land a core row and its outbox row in one statement, the relay that appends the event or enqueues the work and publishes `ENTITY_CHANGED` or `WORK_AVAILABLE` then marks the row done, idempotent on the row's key, and `read_pending(limit)` for the sweep |
...
| `tests/unit/`                          | tenancy on write, the role map, both roots, the tenant-first exceptions list, the import direction (nothing under the OM or infra imports a service or a worker; nothing under infra imports the OM), the contract cases over memory |
