...
description: "Review code or a change through the Network lenses of the Software Design and Architecture Guidelines. Covers The Network Layer, Apps (Push-First Apps), and Client App Architecture (Realtime: One Channel per App): topology, gateway, public types, clients, realtime, push-first. Use for a change that touches this area, or as one leg of arch-review-full."
...
`${CLAUDE_SKILL_DIR}/../../lenses/network.md`. Other perspectives have
...
3. Run the checker that shipped with this lens file, from the root of
   ...
   `python3 "${CLAUDE_SKILL_DIR}/../../checkers/arch_check.py" --group network --format json`.
